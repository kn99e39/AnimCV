...
def run(args) -> dict[str, Any]:
    ...
    if args.resume and jsonl_path.is_file() and jsonl_path.stat().st_size:
        ...
        compare_keys = set(run_identity) - {"record_type", "run_started_utc"}
        mismatched = [key for key in compare_keys if first_line.get(key) != run_identity[key]]
        if mismatched:
    ...
    with jsonl_path.open("a" if args.resume else "w", encoding="utf-8") as stream:
        ...
        for order, position in enumerate(positions):
            ...
            if sample.sample_id in existing:
                continue
            donor_order = int(donor_orders[order])
            donor_position = int(positions[donor_order])
            donor = bank.samples[donor_position]
            real_crop = read_crop(bank, int(position), roots, ADVISOR_CROP_RESOLUTION)
            shuffled_crop = read_crop(bank, donor_position, roots, ADVISOR_CROP_RESOLUTION)
            real_response, real_raw = ask(real_crop)
            shuffled_response, shuffled_raw = ask(shuffled_crop)
            field_position = int(np.where(positions == position)[0][0])
            record = {
                "record_type": "frame",
                ...
                "image_digest_real_crop_rgb": hashlib.sha256(real_crop.tobytes()).hexdigest(),
                ...
                "image_digest_shuffled_crop_rgb": hashlib.sha256(shuffled_crop.tobytes()).hexdigest(),
                "oracle_sign_state": [int(value) for value in reference[field_position]],
                "h0_sign_state": [int(value) for value in h0_signs[field_position]],
    ...
    report = {
        ...
        "elapsed_seconds_this_process": time.perf_counter() - elapsed_started,
    }
